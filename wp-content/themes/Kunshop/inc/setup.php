...
function create_advise_requests_table() {
    global $wpdb;
...
add_action('after_setup_theme', 'create_advise_requests_table');
...
function my_advise_menu() {
    add_menu_page(
        ...
        'advise_management',
        'display_advise_page',
        'dashicons-clipboard', 22 
    );
}
add_action('admin_menu', 'my_advise_menu');

function display_advise_page() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'advise_requests';
    ...
    if (!empty($search)) {
        $where = $wpdb->prepare(
            "WHERE contact_name LIKE %s OR contact_phone LIKE %s OR product_name LIKE %s",
    ...
    ?>
    <div class="wrap">
        <h2>Danh sách yêu tư vấn</h2>

        <form method="get" action="" style="margin-bottom: 20px; margin-top: 20px;">
            <input type="hidden" name="page" value="advise_management">
            <input type="text" name="search" placeholder="Tìm kiếm tên, số điện thoại người yêu cầu..." value="<?php echo esc_attr($search); ?>" />
            <button type="submit" class="button button-primary">Tìm kiếm</button>
        </form>

        <form method="post" action="" style="margin-bottom: 20px; margin-top: 20px; width: 70%">
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                    ...
                    <?php if ($results) : ?>
                        <?php foreach ($results as $item) : ?>
                            <tr>
                                ...
                                <td><a href="<?php echo esc_html($item->product_link); ?>" target="_blank"><?php echo esc_html($item->product_name); ?></a></td>
                                <td><?php echo esc_html($item->created_at); ?></td>
                                <td>
                                    <a href="<?php echo admin_url('admin.php?page=advise_management&delete=' . $item->id); ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa yêu cầu này?')" class="button button-secondary">Xóa</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
                        <tr><td colspan="6">Chưa có yêu cầu tư vấn nào.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
